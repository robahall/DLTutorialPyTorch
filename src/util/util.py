...
def enumerateWithEstimate(
    ...
    for (current_ndx, item) in enumerate(iter):
        ...
        if current_ndx == print_ndx:
            ...
            log.info(f"{desc_str} {current_ndx:-4}/{iter_len}, done at {str(done_dt).rsplit('.', 1)[0]}, {str(done_td).rsplit('.', 1)[0]}")
    ...
    log.warning("{desc_str} ----/{iter_len}, done at {str(datetime.datetime.now()).rsplit('.', 1)[0]}")
